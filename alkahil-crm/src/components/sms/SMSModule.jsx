...
export default function SMSModule() {
  const [selected, setSelected] = useState(smsThreads[0])
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'10px' }}>
          {simPorts.map((port,i) => (
            <motion.div
              ...
              style={{
                background: port.active ? '#F3F2FF' : '#F8F8F8',
                border:`1px solid ${port.active ? '#C4C0F0' : '#E5E7EB'}`,
                borderRadius:'12px', padding:'12px 14px',
              }}
            >
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'8px' }}>
                <span style={{ fontSize:'11px', fontWeight:600, color: port.active ? '#7670C5' : '#888' }}>{port.port}</span>
        ...
        <div style={{
          ...
          <div style={{ padding:'12px', borderBottom:'1px solid #F0F0F0' }}>
            <div style={{
              ...
              <input
                placeholder="Search..."
                style={{ background:'none', border:'none', outline:'none', color:'#111', fontSize:'12px', fontFamily:'inherit', width:'100%' }}
              />
            </div>
          </div>
            ...
            {allSmsConvs.map(conv => (
              <div
                ...
                style={{
                  ...
                  background: selected?.id===conv.id ? '#F3F2FF' : 'transparent',
                  borderLeft: selected?.id===conv.id ? '2px solid #7670C5' : '2px solid transparent',
              ...
              >
                ...
                {conv.unread > 0 && (
                  <div style={{
                    ...
                    background:'#7670C5', color:'#fff', fontSize:'10px',
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'16px', display:'flex', flexDirection:'column', gap:'8px' }}>
                {selected.messages?.map((msg,i) => (
                  <div key={i} style={{ display:'flex', justifyContent: msg.from==='lead' ? 'flex-start' : 'flex-end' }}>
                    <div style={{
                      background: msg.from==='lead' ? '#F5F5F5' : '#7670C5',
