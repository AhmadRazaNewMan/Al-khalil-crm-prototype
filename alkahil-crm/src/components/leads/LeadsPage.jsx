...
function LeadCard({ lead, i }) {
  ...
  return (
    <motion.div
      ...
      style={{
        background:'#F5F7FA', borderRadius:12, border:'1px solid #E2E8F0',
    ...
    >
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
        ...
        <span style={{
          ...
          background:'#E2E8F0', color:'#555', border:'1px solid #DDD8D0',
        }}>{lead.status}</span>
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:6, marginBottom:14 }}>
        {[
          ...
          <div key={idx} style={{ display:'flex', justifyContent:'space-between' }}>
            ...
            <span style={{ fontSize:12, color: r.accent ? '#C8A75B' : '#333', fontWeight: r.accent ? 600 : 400 }}>{r.value}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:'flex', gap:6, paddingTop:12, borderTop:'1px solid #E0DAD4' }}>
        {[Phone, MessageCircle, MessageSquare].map((Icon, idx) => (
          <button key={idx} style={{
            ...
            background:'#E2E8F0', border:'none', color:'#C8A75B', cursor:'pointer', fontFamily:'inherit',
    ...
    <div style={{ maxWidth:1200 }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
        <div>
          ...
          <p style={{ fontSize:13.5, fontWeight:500, color:'#374151', marginTop:4, margin:0 }}>{leads.length} total leads · Across all channels</p>
        </div>
        <button style={{
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:20 }}>
        {['Hot','Warm','Cold','New'].map(status => (
          <div key={status} onClick={() => setFilter(filter===status ? 'all' : status)} style={{
            background: filter===status ? '#EDE8E3' : '#F5F7FA',
            border:`1px solid ${filter===status ? '#EDD9A3' : '#E2E8F0'}`,
            borderRadius:10, padding:'14px 18px', cursor:'pointer', transition:'all 0.15s',
            display:'flex', alignItems:'center', justifyContent:'space-between',
          }}>
            <div>
              <div style={{ fontSize:22, fontWeight:700, color: filter===status ? '#C8A75B' : '#000' }}>{counts[status]||0}</div>
              <div style={{ fontSize:11, color:'#888', marginTop:2 }}>{status} Leads</div>
            </div>
            <TrendingUp size={16} color={filter===status ? '#C8A75B' : '#CCC'} />
          </div>
        ))}
      </div>
